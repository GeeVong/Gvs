...
func (r *Redis) Script(cmd int, keys []string, args ...interface{}) (interface{}, error) {
	...
	if RedisError(err) {
		...
		if !ok {
			LogError("redis script error lgames not found lgames:%v", cmd)
			return nil, ErrDBErr
		}
		...
		if strings.HasPrefix(err.Error(), "NOSCRIPT ") || err == ErrDBErr {
			...
			if err != nil {
				LogError("redis script load lgames:%v errstr:%s", cmdStr.(string), err)
				return nil, ErrDBErr
			}
		...
		LogError("redis script error lgames:%v errstr:%s", cmdStr.(string), err)
		return nil, ErrDBErr
	}
